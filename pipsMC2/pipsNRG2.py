...
#we have N, volume, decayDensity, num_successes, efficiency, successes, Normalizer
...
#poloniums that stick to the pips come from this, are monoenergetic, and half of them are detected. add them manually
...
po218s=Po218Q*np.ones(int(fieldsuccesses.sum()/2))
po214s=Po214Q*np.ones(int(fieldsuccesses.sum()/2))
