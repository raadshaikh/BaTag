...
#we have N, volume, decayDensity, num_successes, efficiency, successes
...
lengths=np.linspace(0,9,2**13) #9cm for good measure
dx=lengths[1]-lengths[0]
alpha_energies=np.zeros(lengths.shape)
alpha_energies[0]=5.5904 #MeV
...
MC_energies_detected=np.interp(MC_distances, lengths, alpha_energies)
...
#hist_of_addition(MC_energies_detected, np.random.normal(0,sigma,len(MC_energies_detected)), bins=FDbins(MC_energies_detected), plot=True)
